...
// Fix: Added ArrowRight to the lucide-react import list.
                            ...
                            {competitors.map(comp => (
                                <div key={comp.id} className="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden hover:shadow-md transition-all flex flex-col group">
                                    <div className="p-8 flex-1">
                                        ...
                                        <h3 className="font-black text-xl text-slate-900 dark:text-white leading-tight uppercase tracking-tighter truncate">{comp.name}</h3>
                                        <p className="text-xs text-blue-500 font-bold truncate mt-1 flex items-center gap-1"><Globe size={12}/> {comp.website || 'Sem site'}</p>
                ...
                <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[1000] flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
                        <div className="p-8 border-b flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                            <div>
                                <h3 className="font-black text-2xl uppercase tracking-tighter">Monitorar Empresa</h3>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Soft Spy Intel</p>
                            </div>
                        ...
                        <form onSubmit={handleAddCompetitor} className="p-8 space-y-6">
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Nome da Empresa</label>
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Website</label>
                                <input className="w-full border-2 border-slate-100 dark:border-slate-800 rounded-2xl p-4 font-bold bg-transparent outline-none focus:border-red-600" placeholder="www.site.com" value={newCompForm.website} onChange={e => setNewCompForm({...newCompForm, website: e.target.value})} />
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Vertical / Atuação</label>
                                <input className="w-full border-2 border-slate-100 dark:border-slate-800 rounded-2xl p-4 font-bold bg-transparent outline-none focus:border-red-600" placeholder="Ex: Software de Logística" value={newCompForm.sector} onChange={e => setNewCompForm({...newCompForm, sector: e.target.value})} />
                            </div>
                            <button type="submit" className="w-full bg-red-600 text-white font-black py-5 rounded-2xl hover:bg-red-700 transition shadow-2xl uppercase tracking-widest text-xs">
                        ...
                        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-900 text-white shrink-0">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">{selectedCompetitor.website} • Intelligence</p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-slate-50 dark:bg-slate-900/50">
                            {isAnalyzing ? (
                                    ...
                                    <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                            <div className="space-y-6">
                                                <div>
                                                    <p className="text-[10px] font-black uppercase text-red-500 tracking-[0.2em] mb-4">Como Ganhar o Negócio (Kill Points)</p>
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-slate-400 text-center">
                                    ...
                                    <p className="font-bold">Análise não encontrada.</p>
                                    <button onClick={() => handleRunAnalysis(selectedCompetitor)} className="mt-4 px-6 py-2 bg-red-600 text-white rounded-xl font-bold">Processar Agora</button>
                                </div>
                            )}
                        </div>
                    </div>
