...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, activeModule, onNavigate }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-slate-900 to-slate-950 text-slate-300 flex flex-col h-[100dvh] shadow-2xl transition-transform duration-300 ease-in-out border-r border-slate-800/50 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-16 px-6 flex items-center justify-between shrink-0 border-b border-slate-800/50 bg-slate-900/50 backdrop-blur-sm">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-lg flex items-center justify-center shadow-lg shadow-blue-500/20 text-white"><span className="font-black text-lg">S</span></div>
        ...
        <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto custom-scrollbar pt-4 pb-4">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => { onNavigate(item.id); onClose(); }} className={`w-full flex items-center justify-between px-4 py-2.5 rounded-lg transition-all duration-200 group ${isActive ? 'bg-blue-600/10 text-blue-400 border border-blue-600/20' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200 border border-transparent'}`}>
        ...
        <div className="px-3 pt-2 pb-0 bg-slate-900 border-t border-slate-800/50 shrink-0">
           ...
           <div className="space-y-1 mt-2 mb-2">
                <button onClick={() => { onNavigate('settings'); onClose(); }} className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeModule === 'settings' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800/50'}`}><Settings size={18} /> Configurações</button>
                <button onClick={toggleTheme} className="w-full flex items-center gap-3 px-4 py-2 text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg text-sm font-medium transition-colors">{theme === 'dark' ? <Sun size={18}/> : <Moon size={18}/>} {theme === 'dark' ? 'Modo Claro' : 'Modo Escuro'}</button>
                <button onClick={handleLogoutClick} className="w-full flex items-center gap-3 px-4 py-2 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg text-sm font-medium transition-colors"><LogOut size={18} /> Sair</button>
           </div>
        </div>
        ...
        <div className="h-8 bg-black/30 border-t border-black/20 flex items-center justify-between px-4 text-[10px] text-slate-500 select-none shrink-0">
            ...
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => refreshData()} disabled={isSyncing} className="hover:text-white transition disabled:opacity-30"><RefreshCw size={10}/></button>
            </div>
        </div>
      </div>
